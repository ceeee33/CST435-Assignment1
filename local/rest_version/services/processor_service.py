...
UPLOADER_URL = os.getenv("UPLOADER_URL", "http://local-rest-uploader:50060/upload")
NOISE_URL = os.getenv("NOISE_URL", "http://local-rest-noise:50061/reduce")
BACKGROUND_URL = os.getenv("BACKGROUND_URL", "http://local-rest-background:50062/remove")
CLASSIFIER_URL = os.getenv("CLASSIFIER_URL", "http://local-rest-classifer:50063/classify")
...
async def get_status(task_id: str):
    ...
    if "response" in resp_future or "error" in resp_future:
        ...
        if all_stages_done and timeline:
            ...
            table = Table(
                title=f"📊 Pipeline Performance Summary - {file_name}",
                show_header=True,
